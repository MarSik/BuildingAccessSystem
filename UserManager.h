...
{
public:
    static uint16_t GetMaxUsers()
    {
        return 0; // TODO fix count
    }
    
    static void DeleteAllUsers()
    {
        for (uint16_t i=0; i<GetMaxUsers(); i++)
        {
            // Delete sizeof(kUser)
        }
    }
...
private:
    ...
    static bool WriteUserAt(int s32_Index, kUser* pk_User)
    {
        ...
        {
            EEPROM.write(P + i, pu8_Ptr[i]); // TODO fix eeprom write
        }
        return true;
    }
    ...
    static bool ReadUserAt(int s32_Index, kUser* pk_User)
    {
        ...
        for (uint32_t i=0; i<sizeof(kUser); i++)
        {
            pu8_Ptr[i] = EEPROM.read(P + i); // TODO fix eeprom read
        }
        return true;
    }
